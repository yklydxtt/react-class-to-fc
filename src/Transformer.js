...
class Transformer {
    ...
    traverse(ast, {
      ClassDeclaration(path) {
        ...
        _self.state.forEach((item) => {
          const decl = t.arrayPattern([
            t.identifier(item.key),
            t.identifier(`set${item.key[0].toUpperCase()}${item.key.slice(1)}`)
          ])
          const call = t.callExpression(t.identifier('useState'), [item.value])
          // 防止命名重复的问题，暂时用var
          _self.componentBody.unshift(
            t.variableDeclaration('var', [t.variableDeclarator(decl, call)])
          )
        })
        ...
        if (
          ...
          for (let state in states) {
            const statement = t.expressionStatement(
              t.callExpression(
                t.identifier(`set${state[0].toUpperCase()}${state.slice(1)}`),
                [states[state]]
              )
            )
          ...
          blockStatement.replaceWith &&
            blockStatement.replaceWith(...blockBodyList)
        } else if (
          node.object.type === 'ThisExpression' &&
  ...
  handleClassFn = (path) => {
    ...
    if (cycle.indexOf(methodName) === -1) {
      ...
      this.componentBody.push(
        t.functionDeclaration(node.key, params, node.body)
      )
    ...
    return {
      Program(path) {
        ...
        _self.outerVariable.forEach((item) => {
          const variable = t.variableDeclaration('let', [
        ...
        if (node.source.value === 'react') {
          node.specifiers = node.specifiers.filter(
            (item) =>
              ...
              unusedImports.indexOf(item.imported.name) === -1
          )
